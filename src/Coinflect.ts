...
export default class Coinflect {
  ...
  constructor(transport: Transport, scrambleKey: string = "Coinflect", logger: (msg: string) => void = console.error) {
   ...
   * @example
   * await coinflect.getWalletPublicKey("44'/9000'/0'/0/0");
   */
  async getWalletAddress(derivation_path: string, hrp = ""): Promise<Buffer> {
    ...
    const p1 = hrp.length;
    const p2 = 0x00;
    const data = Buffer.concat([Buffer.from(hrp, "latin1"), this.encodeBip32Path(BIPPath.fromString(derivation_path))]);
   ...
   * @example
   * await coinflect.getWalletExtendedPublicKey("44'/9000'/0'/0/0");
   */
   ...
   * @example
   * const signatures = await coinflect.signHash(
   ...
   * @example
   * const signatures = await coinflect.signTransaction(
   *   BIPPath.fromString("44'/9000'/0'"),
   *   [BIPPath.fromString("0/0")],
   *   Buffer.from("...", "hex"),
  ...
  async signTransaction(
    ...
    if (changePath != undefined && changePath != null) {
      const preamble_ = Buffer.concat([preamble, this.encodeBip32Path(changePath)]);
    ...
    while (remainingData.length > 0) {
      ...
      response = await this.transport.send(
        this.CLA,
        this.INS_SIGN_TRANSACTION,
        remainingData.length > 0 ? SIGN_TRANSACTION_SECTION_PAYLOAD_CHUNK : SIGN_TRANSACTION_SECTION_PAYLOAD_CHUNK_LAST,
        0x00,
        thisChunk
      );
    }
  ...
  /**
   * Get the version of the Coinflect app installed on the hardware device
   *
   ...
   * @example
   * console.log((await coinflect.getWalletId()).toString("hex"));
   *
   * 79c46bc3
   */
